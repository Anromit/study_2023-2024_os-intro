...
- Попытка получить с клиента доступ к серверу посредством SSH-соединения через пользователя arromichina.
...
- Повторная попытка получения доступа с клиента к серверу посредством SSH-соединения через пользователя arromichina.
...
- Перезапуск sshd и повторная попытка получить доступ с клиента к серверу посредством SSH- соединения через пользователя arromichina.
...
- Попытка получить с клиента доступа к серверу посредством SSH-соединения через пользователя arromichina и получение доступа root.

![](./image/16.png)

## Настройка дополнительных портов для удалённого доступа по SSH

- Повторная попытка получения доступа с клиента к серверу посредством SSH-соединения через пользователя arromichina, указав порт 2022 Получение доступа root.
...
- Просмотр на клиенте запущенных служб с протоколом TCP и перенаправление порта 80 на server.arromichina.net на порт 8080 на локальной машине.
...
- Открытие на клиенте терминала под пользователем arromichina. Просмотр имени узла сервера, списка файлов на сервере и почты на сервере.
